<template>
	<view class="page mine-page">
		<view class="header" :class="role==='employer' ? 'is-employer' : ''">
			<view class="row">
			...
			<!-- 雇主核心数据 / 学生认证状态 -->
			<view v-if="role==='employer'" class="stats">
				<view class="stat">
		...
		<view class="quick">
			<view class="quick-item" @click="goMyJobs" v-if="role==='employer'">
				<text class="ico">📋</text>
				<text class="t">我的岗位</text>
			</view>
			<view class="quick-item" @click="goPublish" v-if="role==='employer'">
				<text class="ico">➕</text>
				<text class="t">发布岗位</text>
			</view>
			<view class="quick-item" @click="goProfile" v-if="role==='employer'">
				<text class="ico">🏪</text>
				<text class="t">店铺资料</text>
			</view>
		...
		<view class="menu card">
			<view class="menu-row" @click="goCert">
				<text class="lbl">实名认证</text>
				<text class="val">{{ certText }}</text>
			</view>
			<view class="menu-row" @click="goProfile">
				<text class="lbl">{{ role==='employer' ? '店铺资料' : '个人资料' }}</text>
				<text class="arrow">›</text>
			</view>
...
<script>
	...
	import { confirm, toastSuccess } from '@/utils/ui'
	...
	export default {
		...
		computed: {
			...
			roleText() {
				const map = { student: '学生', employer: '雇主', admin: '管理员' }
		...
		methods: {
			...
			onSwitchRole() {
				...
				uni.showActionSheet({
					itemList: list.map(r => ({ student: '学生', employer: '雇主' }[r] || r)),
					success: r => {
